...
#define PRINT_NONOGRAMA for(int i{0}; i < nonograma.size(); ++i){for(int j{0}; j < nonograma[0].size(); ++j){if(nonograma[i][j] == 1){out_file << "O";}/*else if(nonograma[i][j] == 0){std::cout << "X";}*/else{out_file << " ";}}out_file << std::endl;}
...
int main(int argc, char * argv[]){
    ...
    for(int times = 0; times < 10; ++times){
        ...
        for(int repetitions = 0; repetitions < N_REPETITIONS; ++ repetitions){
            ...
            if(unsolvable){
                // out_file << "Sem solução :D" << std::endl;
            }
            else if(repetitions == N_REPETITIONS - 1){
        ...
        out_file << "# Medição do tempo do teste "<< times << ": " << std::to_string(elapsed_time_mean) << "ms com tempo minimo de " << std::to_string(time_min) << "ms e tempo máximo de" << std::to_string(time_max) <<"ms.\n";
